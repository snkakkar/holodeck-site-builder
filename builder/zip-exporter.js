...
(function (global) {
  ...
  function generateReadme(state) {
    ...
    return [
      ...
      "",
      "Embedded CX component slides render AubreyDemo URLs inside an iframe. If",
      "a component refuses to load (some sites block embedding through",
      "`X-Frame-Options` or CSP), the slide will show an **Open in new tab**",
      "button as a fallback. To replace or add component URLs, edit the",
      "`builderPlan.cxComponents` array in `holodeck.config.js` — the `url` field",
      "controls what's embedded, and the `deviceFrame` field controls the",
      "phone / desktop / tablet chrome.",
      "",
    ].join("\n");
  }
