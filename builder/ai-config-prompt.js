...
- Shopper Agent:           https://aubreydemo.com/scene/1060/frame  (agent, mobile)`;
  ...
  const STORY_PARSE_PROMPT = [
    ...
    "   Write each as a COMPLETE thought that FITS its budget; do NOT rely on the",
    "   renderer to trim. Over-budget copy is cut back to the last WHOLE sentence (or,",
    "   for a long single sentence, to the last clause) that fits — so a trailing idea",
    "   you counted on is silently DROPPED, and a too-long field shows less than you",
    "   wrote. Respect the ≤N-char/word hints shown in the template above.",
    ...
    "   SMALLEST slot that field appears in — because the same field is reused on",
    "   several slides at tighter widths than its max:",
    "     • businessProblem — max 220, but also renders as an exec column at ~180 and",
    "       a stat chip at ~46; make the FIRST sentence ≤ ~150 and self-contained.",
    "     • currentStatePain / futureStateVision — max 180; first sentence ≤ ~150.",
    "     • storyActs[].summary — max 200, but also renders as a scene sub-line at ~150",
    "       and a journey-circle line; write ONE sentence ≤ ~150 that stands alone.",
    "   If a field genuinely needs two sentences, make EACH one self-contained — never",
    "   split one idea across the budget boundary, because only the sentences (or the",
    "   leading clause) that fit are shown; the rest is dropped.",
